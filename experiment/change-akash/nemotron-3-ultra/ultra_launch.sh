...
# LOCAL PATCH: `:-` substitutes the default when the var is unset *or empty*, so
# there was no way to opt out of the sandbox from the environment. `-` substitutes
# only when unset, letting a caller pass SANDBOX_COMMAND="" to disable it.
# ray.sub gates the sandbox on SANDBOX_CONTAINER && SANDBOX_COMMAND both being
# non-empty (ray.sub:559), so an empty command skips the ports dir, the 64-ready
# wait, and the sandbox srun entirely.
